...
async def do_direct_handshake(proto_tag, dc_idx, dec_key_and_iv=None):
    ...
    # --- Modified IPv6 Check ---
    use_ipv6 = config.ENABLE_IPV6 and my_ip_info["ipv6"]
    if use_ipv6:
    # --- End Modified IPv6 Check ---
    ...
    try:
        ...
        # Use the selected IP address (dc) in the error message
        print_err("The Telegram server connection is bad: %d (%s %d) %s" % (dc_idx + 1, dc, TG_DATACENTER_PORT, E))
        return False
    except (OSError, asyncio.TimeoutError) as E:
...
async def do_middleproxy_handshake(proto_tag, dc_idx, cl_ip, cl_port):
    ...
    # --- Modified IPv6 Check ---
    use_ipv6_tg = config.ENABLE_IPV6 and my_ip_info["ipv6"]
    # --- End Modified IPv6 Check ---
...
def init_ip_info():
    ...
    if config.ENABLE_IPV6:
        ...
        if my_ip_info["ipv6"]:
             print_err("IPv6 enabled and detected, will use if possible.")
    else:
         my_ip_info["ipv6"] = None # Ensure it's None if disabled
         print_err("IPv6 is disabled by configuration.")
    # --- End Modified IPv6 Check ---

    if config.USE_MIDDLE_PROXY:
        if not my_ip_info["ipv4"] and not my_ip_info["ipv6"]:
            print_err("Failed to determine your public IP (v4 or enabled v6), advertising disabled")
            disable_middle_proxy = True
...
def create_servers(loop):
    ...
    if config.LISTEN_ADDR_IPV4:
        print_err("Starting IPv4 server on %s:%d" % (config.LISTEN_ADDR_IPV4, config.PORT))
    ...
    # --- Modified IPv6 Check ---
    if config.ENABLE_IPV6 and config.LISTEN_ADDR_IPV6 and socket.has_ipv6:
        print_err("Starting IPv6 server on [%s]:%d" % (config.LISTEN_ADDR_IPV6, config.PORT))
    # --- End Modified IPv6 Check ---
        task = asyncio.start_server(handle_client_wrapper, config.LISTEN_ADDR_IPV6, config.PORT,
                                    limit=get_to_tg_bufsize(), reuse_port=reuse_port)
        servers.append(loop.run_until_complete(task))
    elif config.LISTEN_ADDR_IPV6 and not config.ENABLE_IPV6:
        print_err("IPv6 listener configured but ENABLE_IPV6 is False. Skipping IPv6 server.")

    if config.LISTEN_UNIX_SOCK and has_unix:
        remove_unix_socket(config.LISTEN_UNIX_SOCK)
        print_err("Starting Unix socket server on %s" % config.LISTEN_UNIX_SOCK)
    ...
    if config.METRICS_PORT is not None:
        if config.METRICS_LISTEN_ADDR_IPV4:
            print_err("Starting IPv4 metrics server on %s:%d" % (config.METRICS_LISTEN_ADDR_IPV4, config.METRICS_PORT))
            task = asyncio.start_server(handle_metrics, config.METRICS_LISTEN_ADDR_IPV4,
                                        config.METRICS_PORT)
            servers.append(loop.run_until_complete(task))
        # --- Modified IPv6 Check ---
        if config.ENABLE_IPV6 and config.METRICS_LISTEN_ADDR_IPV6 and socket.has_ipv6:
            print_err("Starting IPv6 metrics server on [%s]:%d" % (config.METRICS_LISTEN_ADDR_IPV6, config.METRICS_PORT))
        # --- End Modified IPv6 Check ---
            task = asyncio.start_server(handle_metrics, config.METRICS_LISTEN_ADDR_IPV6,
                                        config.METRICS_PORT)
            servers.append(loop.run_until_complete(task))
        elif config.METRICS_LISTEN_ADDR_IPV6 and not config.ENABLE_IPV6:
            print_err("IPv6 metrics listener configured but ENABLE_IPV6 is False. Skipping IPv6 metrics server.")
